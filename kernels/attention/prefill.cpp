...
// Apodex Q1120/Q4480, K<=262144, H16/KV2/D256; each call is causal over its final Q queries.
...
template<bool IsQ, int KB, int Q> __global__ __launch_bounds__(256) void prep_qk(
...
// Copyright 2026 Ciru. Fixed-Q prefill adaptation of Ciru's fused-D256
// decode consumer. Keep the released prefill's QK/PV arithmetic and packing.
// QK/P is shared across two D128 consumers; no joint-DC approximation added.
template<int KB, int Q> __global__ __launch_bounds__(256) void attention(const uint32_t* qp,const __half* qs,
    ...
    __shared__ uint32_t spp[64*4];
    __shared__ float salpha[64],spscale[64],snorm[64];
    int tid=threadIdx.x,lane=tid&31,row16=lane&15,piece=lane>>4;
    int wave=(tid>>5)&3,dhalf=tid/128,folded=wave*16+row16;
    int h=blockIdx.y,kh=h/8,row=blockIdx.x*64+folded;
    bool live=row<Q;
    float sq=(dhalf==0&&live)?__half2float(qs[h*Q+row]):1.f;
    uint32_t q[32];
    if(dhalf==0){
#pragma unroll
...
#pragma unroll
            ...
            float next=fmaxf(m,mx),sum=0;
            alpha=exp2f((m-next)*1.4426950408889634f);
#pragma unroll
...
#pragma unroll
                ...
                salpha[folded]=alpha;spscale[folded]=sp;snorm[folded]=l;
            }
        }
        __syncthreads();
        if(dhalf!=0){
#pragma unroll
            ...
            alpha=salpha[folded];sp=spscale[folded];
        }
#pragma unroll
...
#pragma unroll
            for(int j=0;j<8;++j)accum[tile*8+j]+=float(dots[j])*sp*svs[dhalf*128+tile*16+2*j+piece];
        }
        __syncthreads();
    }
    ...
    if(live){
#pragma unroll
        for(int j=0;j<64;++j)out[(size_t(row)*16+h)*256+dhalf*128+2*j+piece]=bf16(accum[j]/l);
    }
}

template<int KB, int Q> int prepare(const void* q,const void* k,const void* v,void* qp,void* qs,void* kp,void* ks,void* vp,void* vs,int tokens,int q_stride0,int q_stride1,void* stream){
    auto s=reinterpret_cast<hipStream_t>(stream);
    prep_qk<true,KB,Q><<<(Q*16+7)/8,256,0,s>>>((const bf16*)q,(uint32_t*)qp,(__half*)qs,tokens,q_stride0,q_stride1);
    prep_qk<false,KB,Q><<<(tokens*2+7)/8,256,0,s>>>((const bf16*)k,(uint32_t*)kp,(__half*)ks,tokens,512,256);
...
extern "C" __attribute__((visibility("default"))) int iu4_prepare(
    ...
    if(tokens<1120 || tokens>262144 || tokens%32) return int(hipErrorInvalidValue);
    if(tokens<=65536)return prepare<2048,1120>(q,k,v,qp,qs,kp,ks,vp,vs,tokens,q_stride0,q_stride1,stream);
    return prepare<8192,1120>(q,k,v,qp,qs,kp,ks,vp,vs,tokens,q_stride0,q_stride1,stream);
}
extern "C" __attribute__((visibility("default"))) int iu4_attention(
    ...
    if(tokens<1120 || tokens>262144 || tokens%32) return int(hipErrorInvalidValue);
    auto s=reinterpret_cast<hipStream_t>(stream);
    if(tokens<=65536)attention<2048,1120><<<dim3((1120+63)/64,16),256,0,s>>>((const uint32_t*)qp,(const __half*)qs,(const uint32_t*)kp,(const __half*)ks,(const uint32_t*)vp,(const __half*)vs,(bf16*)output,tokens);
    else attention<8192,1120><<<dim3((1120+63)/64,16),256,0,s>>>((const uint32_t*)qp,(const __half*)qs,(const uint32_t*)kp,(const __half*)ks,(const uint32_t*)vp,(const __half*)vs,(bf16*)output,tokens);
    return int(hipGetLastError());
}

extern "C" __attribute__((visibility("default"))) int iu4_prepare_q4480(
    const void* q,const void* k,const void* v,void* qp,void* qs,void* kp,void* ks,void* vp,void* vs,int tokens,int q_stride0,int q_stride1,void* stream){
    if(tokens<4480 || tokens>262144 || tokens%32) return int(hipErrorInvalidValue);
    if(tokens<=65536)return prepare<2048,4480>(q,k,v,qp,qs,kp,ks,vp,vs,tokens,q_stride0,q_stride1,stream);
    return prepare<8192,4480>(q,k,v,qp,qs,kp,ks,vp,vs,tokens,q_stride0,q_stride1,stream);
}
extern "C" __attribute__((visibility("default"))) int iu4_attention_q4480(
    const void* qp,const void* qs,const void* kp,const void* ks,const void* vp,const void* vs,void* output,int tokens,void* stream){
    if(tokens<4480 || tokens>262144 || tokens%32) return int(hipErrorInvalidValue);
    auto s=reinterpret_cast<hipStream_t>(stream);
    if(tokens<=65536)attention<2048,4480><<<dim3((4480+63)/64,16),256,0,s>>>((const uint32_t*)qp,(const __half*)qs,(const uint32_t*)kp,(const __half*)ks,(const uint32_t*)vp,(const __half*)vs,(bf16*)output,tokens);
    else attention<8192,4480><<<dim3((4480+63)/64,16),256,0,s>>>((const uint32_t*)qp,(const __half*)qs,(const uint32_t*)kp,(const __half*)ks,(const uint32_t*)vp,(const __half*)vs,(bf16*)output,tokens);
    return int(hipGetLastError());
}

extern "C" __attribute__((visibility("default"))) int iu4_fused_d256(){return 64;}
