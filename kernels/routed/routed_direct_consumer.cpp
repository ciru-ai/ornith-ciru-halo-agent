...
#pragma unroll
        for(int j=0;j<Words/2;++j){
            ...
            int w=int(codes[wbase+((size_t(n/16)*Groups+group)*Words+local)*16+(n&15)]);
        ...
        if(piece==0){
            ...
            Meta wm=metadata[mbase+(size_t(n/16)*Groups+group)*16+(n&15)];
...
#include "routed_direct_prefill.h"
...
#define PROJECT(N,K,GATE,CODES,META,LO,HI,SCALES,SUMS,Y) \
    if(T<=8){projection<N,K,GATE,A8><<<R*(N/16),32,0,stream>>>(rid,static_cast<const uint32_t*>(CODES),static_cast<const Meta*>(META),at<uint32_t>(workspace,l.LO),at<uint32_t>(workspace,l.HI),at<__half>(workspace,l.SCALES),at<int32_t>(workspace,l.SUMS),at<float>(workspace,l.Y),sticky);} \
