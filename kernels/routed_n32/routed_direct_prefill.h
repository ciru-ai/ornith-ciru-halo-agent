// Copyright (c) Ciru. Bounded grouped prefill for the G256/A8 and optional A4 ABI.
...
template<int N,int K,bool Gate,bool A8> __device__ __forceinline__ void wmma_job(
    ...
    size_t bankbase=(size_t(desc.expert)*(N/16)+tile)*Groups;
    float values[8]={};
...
#pragma unroll
        for(int step=0;step<Words/2;++step){
            ...
            size_t wi=((bankbase+group)*Words+2*step)*16+lane16;
            i32x2 w={int32_t(codes[wi]),int32_t(codes[wi+16])};
        ...
        uint32_t own_meta=0;
        if(piece==0)own_meta=reinterpret_cast<const uint32_t*>(metadata)[(bankbase+group)*16+lane16];
        uint32_t wm_words[8];
#pragma unroll
...
template<int N,int K,bool Gate,bool A8> __global__ __launch_bounds__(32) void grouped_projection(
    ...
    uint32_t nw=queue[0],nc=queue[1],wjobs=nw*Tiles,jobs=wjobs+nc*Tiles;
    for(uint32_t job=blockIdx.x;job<jobs;job+=gridDim.x){
        ...
        else{uint32_t local=job-wjobs;projection_job<N,K,Gate,A8>(ids,codes,metadata,low,high,scales,sums,output,sticky,compact[local/Tiles],local%Tiles);}
    }
}
} // namespace routed_direct
