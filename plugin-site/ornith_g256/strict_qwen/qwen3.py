...
from .qwen3_contract import Qwen3ContractEngine, validate_qwen3_slots
...
class Qwen3Parser(ParserEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.CONFIG_NAME == "qwen3":
            self._engine = Qwen3ContractEngine(
                self.parser_engine_config, tokenizer, vocab=self.vocab
            )
        vocab = self.vocab
    ...
    def _check_skip_tool_parsing(self, request):
        ...
        self._contract_tool_choice = getattr(request, "tool_choice", None)

    def _validate_tool_contract(self):
        if (
            isinstance(self._engine, Qwen3ContractEngine)
            and not self.skip_tool_parsing
            and not self._suppress_tool_calls
            and bool(self._tools)
        ):
            self._engine.validate_contract_frames()
            validate_qwen3_slots(self, _qwen3_arg_converter)
            choice = getattr(self, "_contract_tool_choice", None)
            named = getattr(getattr(choice, "function", None), "name", None)
            if choice == "required" or named is not None:
                from .qwen3_contract import Qwen3ToolContractError
                if not self._tool_slots:
                    raise Qwen3ToolContractError("required tool call missing")
                if named is not None and any(s.name != named for s in self._tool_slots):
                    raise Qwen3ToolContractError("tool call does not match named choice")
    ...
    def parse_delta(self, *args, **kwargs):
        delta = super().parse_delta(*args, **kwargs)
        if kwargs.get("finished"):
            self._validate_tool_contract()
        return delta

    def _single_pass_parse(self, *args, **kwargs):
        result = super()._single_pass_parse(*args, **kwargs)
        self._validate_tool_contract()
        return result
