...
class GPUModelRunner(
    ...
    def __init__(
        ...
        # DFlash target features can span PP stages.  Every stage must enable
        # the target's aux-state taps so earlier-stage features can ride the
        # normal IntermediateTensors transfer to the last-rank drafter.
        self.use_aux_hidden_state_outputs = bool(
            self.speculative_config and self.speculative_config.use_dflash()
        )
        ...
        # external_launcher PP broadcasts the last rank's logits to every rank,
        # and every rank runs sample_tokens() against its local scheduler state.
        # Keep the heavyweight drafter on the last rank, but give each rank the
        # lightweight verifier used by _sample().
    ...
    def _build_attention_metadata(
        ...
        for kv_cache_gid, kv_cache_group in enumerate(kv_cache_groups):
            ...
            if hasattr(self, "drafter") and spec_decode_common_attn_metadata is None:
                if isinstance(
                    self.drafter,
                    (
                        EagleProposer,
            ...
            elif hasattr(self, "drafter") and isinstance(
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("gpu_model_runner: postprocess"):
            if self.use_aux_hidden_state_outputs and get_pp_group().is_last_rank:
                # The last PP rank receives any upstream aux features and
                # returns the complete ordered feature list. Earlier ranks
                # pack their local features into IntermediateTensors instead.
    ...
    def sample_tokens(
        ...
        # The drafter exists only on the last PP rank. Proposed ids are
        # relayed to peer schedulers in take_draft_token_ids().
    ...
    def take_draft_token_ids(self) -> DraftTokenIds | None:
        ...
        pp = get_pp_group()
        output: DraftTokenIds | None = None
        if pp.is_last_rank and self._draft_token_req_ids:
            draft_token_ids, req_ids = self._get_draft_token_ids_cpu()
            output = DraftTokenIds(req_ids, draft_token_ids)

        # Every external_launcher rank owns an independent sync scheduler.
        if self.broadcast_pp_output:
            relayed = pp.broadcast_object(
                output if pp.is_last_rank else None,
                src=pp.world_size - 1,
            )
            assert relayed is None or isinstance(relayed, DraftTokenIds)
            if relayed is not None and not pp.is_last_rank:
                # External-launcher engines create independent request IDs on
                # each rank. Their persistent batches remain row-aligned, so
                # attach the relayed draft rows to this scheduler's local IDs.
                local_req_ids = self.input_batch.req_ids.copy()
                if not (
                    len(relayed.req_ids)
                    == len(relayed.draft_token_ids)
                    == len(local_req_ids)
                ):
                    raise RuntimeError(
                        "PP draft relay row mismatch: "
                        f"local={len(local_req_ids)}, "
                        f"remote_ids={len(relayed.req_ids)}, "
                        f"remote={len(relayed.draft_token_ids)}"
                    )
                relayed = DraftTokenIds(local_req_ids, relayed.draft_token_ids)
            return relayed
