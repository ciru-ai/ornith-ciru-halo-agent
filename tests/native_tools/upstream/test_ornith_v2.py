...
MODEL = '/srv/ssd/sn850x/scratch/crown/ornith-hermes-20260909/bundle/models/target'
...
def real_grammar(i):
    matcher = xgr.GrammarMatcher(ctx, max_rollback_tokens=32)
...
# Ornith DFlash compatibility: cover full windows rather than truncating MTP4.
compatibility = []
for spec in [7, 15]:
    for kind in ['uniform', 'ragged', 'invalid', 'reasoning']:
        drafts = {str(i): ([1, 2, 3, 4, 5, 6] * 3)[:spec if kind != 'ragged' else (i * 3) % (spec + 1)] for i in range(8)}
        if kind == 'invalid':
            drafts['2'][2] = -1
            drafts['4'][3] = -1
        if kind == 'reasoning':
            for i in range(1, 8): drafts[str(i)][3] = 99
        check_fake(f'Ornith DF{spec} {kind}', spec=spec, drafts=drafts,
                   thinking=set(range(1, 8)) if kind == 'reasoning' else None)
    actual_draft = tokenizer.encode('to give the material a satin finish with a subtle warm reflection across the rounded corners and a muted brushed texture', add_special_tokens=False)[:spec]
    assert len(actual_draft) == spec
    for count in [1, 2, 8]:
        for window in ['uniform', 'ragged']:
            a, c = manager(vocab=vocab, spec=spec), manager(vocab=vocab, spec=spec)
            ar, cr = requests(real_grammar, count), requests(real_grammar, count)
            ids = list(ar)
            scheduled = {i: actual_draft if window == 'uniform' else actual_draft[:(int(i) * 3) % (spec + 1)] for i in ids}
            expected = original(a, ar, ids, scheduled).copy()
            actual = wrapped(c, cr, ids, scheduled).copy()
            assert np.array_equal(actual, expected), (spec, count, window)
            assert bool(c._ciru_mask_state.parallel_batches) == (count > 1)
            for i in ids:
                ga, gc = ar[i].structured_output_request.grammar, cr[i].structured_output_request.grammar
                assert ga.num_processed_tokens == gc.num_processed_tokens == 0
                # Equal next masks after a committed token verifies matcher rollback,
                # not merely the Python processed-token counter.
                assert ga.accept_tokens(i, actual_draft[:1]) == gc.accept_tokens(i, actual_draft[:1])
            next_a = original(a, ar, ids, {}).copy()
            next_c = wrapped(c, cr, ids, {}).copy()
            assert np.array_equal(next_a, next_c)
            row = dict(spec=spec, count=count, window=window, draft_lengths={i: len(v) for i,v in scheduled.items()}, mask_equal=True, committed_next_mask_equal=True)
            compatibility.append(row)
            print(json.dumps(row), flush=True)
            shutdown(c)
(ROOT / 'ornith-compatibility.json').write_text(json.dumps(dict(passed=True, model=MODEL, real_cases=compatibility, fake_cases=8, gpu_used=False, serving_speed_claim=False), indent=2)+'\n')
